...
- **CSS는 항상 최상위(`globals.css` 또는 디자인 토큰 변수)에서 참조하여 적용한다** — 컴포넌트 내 인라인 스타일 또는 임의 값 직접 작성 금지 / 하드코딩 금지
...
## 접근성 규칙 (Accessibility)

### 필수 준수 항목
- **아이콘 전용 버튼은 반드시 `aria-label` 추가** — 텍스트 없는 `<button>`, `<a>` 전부 해당
  ```tsx
  <button aria-label={t('common.openMenu')}><Menu /></button>
  ```
- **토글 버튼에는 `aria-expanded` 추가** — 드롭다운, 아코디언, 메뉴 등
...
```
흰 배경 기준 사용 가능한 최소 색상:
  본문 텍스트    → text-secondary-700 이상  (#4e5968, 대비 6.9:1) ✅
  보조 텍스트    → text-secondary-600 이상  (#6b7684, 대비 4.41:1) ⚠️ 소체 주의
  text-secondary-500 (#8b95a1, 대비 3.0:1) → 흰 배경 텍스트 사용 금지 ❌
```
...
- **개발 도구는 static import 금지** — `ReactQueryDevtools` 등은 `dynamic import` 사용
  ```ts
  // ❌ 잘못된 방식: 프로덕션 번들에 포함됨
  ...
  const ReactQueryDevtools = process.env.NODE_ENV === 'development'
    ...
    : () => null;
  ```
- **무거운 모달/탭 컴포넌트는 `next/dynamic` lazy loading** — 초기 번들에서 제외
- **Lighthouse 성능 측정은 반드시 프로덕션 빌드에서** — `npm run build && npm run start`
  - `npm run dev` 측정 결과는 참고 불가 (devtools 포함, 미압축)
- **대시보드 등 데이터 의존 페이지는 섹션별 독립 로딩** — 전체 isLoading 블로킹 금지
  - 각 쿼리의 loading 상태를 별도로 노출해 카드/섹션별 스켈레톤 표시
...
- `any` 타입 사용 금지
- 아이콘 전용 버튼에 `aria-label` 누락 금지
- `maximumScale` viewport 설정 금지 (WCAG 위반)
- `text-secondary-500` 흰 배경 텍스트 사용 금지 (대비 3.0:1)
- 헤딩 계층 건너뜀 금지 (h1→h3 등)
- 개발 도구 라이브러리 static import 금지 → dynamic import 사용
