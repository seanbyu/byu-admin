...
export const SalesRegistrationModal = memo(function SalesRegistrationModal({
  ...
  const menuMap = useMemo(() => {
    const map: Record<string, { id: string; category_id: string; name: string; duration_minutes?: number; base_price?: number; price?: number }> = {};
  ...
  const handleSave = useCallback(() => {
    ...
    onClose();
  }, [booking, selectedServiceIds, totalServicePrice, paymentMethod, productAmount, productName, menuMap, categoryNameMap, onSave, onClose, toast, isSalesRegistered, t]);
  ...
  return (
    ...
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={isSalesRegistered ? t('booking.salesModal.editTitle') : t('booking.salesModal.title')}
      size="sm"
    >
      <div className="space-y-4 text-secondary-800">
          ...
          {selectedServiceGroups.length > 0 && (
            <div className="mb-3 space-y-1.5 rounded-lg border border-primary-200 bg-white p-2.5">
              ...
              <div className="flex items-center justify-between border-t border-primary-100 pt-2 text-xs text-secondary-700">
                ...
                <span>
                  {totalServiceDuration}min / ฿{selectedServiceGroups.reduce((s, g) => s + g.totalPrice, 0).toLocaleString()}
                </span>
              </div>
            </div>
          )}
        ...
        {/* 버튼 */}
        <div className={`flex ${isSalesRegistered ? 'justify-between' : 'justify-end'} pt-2`}>
          {isSalesRegistered && (
            <Button
              variant="danger"
              type="button"
