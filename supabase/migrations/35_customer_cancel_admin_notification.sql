-- ============================================
-- 35_customer_cancel_admin_notification.sql
--
-- 변경:
--   1. trg_on_booking_status_changed 재정의
--      - 어드민이 직접 취소한 경우 (cancelled_by = 'ADMIN') 어드민 IN_APP 알림 생성 안 함
--      - 고객이 취소한 경우 (cancelled_by 없거나 'CUSTOMER') 어드민 IN_APP 알림 생성
--      - 알림 metadata에 cancelled_by 필드 추가
-- ============================================
...
BEGIN
  ...
  -- 어드민이 직접 취소한 경우(cancelled_by = 'ADMIN')는 알림 생성 안 함
  -- 고객이 취소한 경우(cancelled_by = 'CUSTOMER' or 없음)만 어드민에게 알림
